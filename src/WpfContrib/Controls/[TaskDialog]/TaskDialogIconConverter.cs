...
namespace Avalon.Windows.Controls
{
    ...
    public class TaskDialogIconConverter : ImageSourceConverter
    {
        ...
        private static readonly string _iconUriScheme;
        private static readonly Regex _iconRegex = new Regex(@"component/Images/(\w+)\.ico", RegexOptions.IgnoreCase);
            ...
            Type destinationType)
        {
            ...
            {
                Match match = _iconRegex.Match(value.ToString());
                if (match.Success)
                {
                    TaskDialogIcon icon;
                    if (Enum.TryParse(match.Groups[1].Value, out icon))
                    {
                        return icon;
                    }
                }
